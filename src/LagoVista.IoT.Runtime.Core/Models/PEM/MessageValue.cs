        ...
        public ValidationResult Validate()
        {
            ...
            switch (Type.Value)
            {
                ...
                case ParameterTypes.Decimal:
                    if (!decimal.TryParse(Value, out decimal decValue)) result.Errors.Add(Resources.ErrorCodes.Messaging.ValueDecimalINotDecimal.ToErrorMessage($"Key={Key},Value={Value}"));
                    break;
                case ParameterTypes.GeoLocation:
                    ...
                    if (!match.Success)
                    {
                        result.Errors.Add(Resources.ErrorCodes.Messaging.InvalidGeoLocationFormation.ToErrorMessage($"Key={Key},Value={Value}"));
                    }
                    else
                    {
                        if (double.TryParse(match.Groups["lat"].Value, out double lat) && double.TryParse(match.Groups["lon"].Value, out double lon))
                        {
                            if (lat > 90 || lat < -90) result.Errors.Add(Resources.ErrorCodes.Messaging.InvalidGeoLocationFormation.ToErrorMessage($"Key={Key},Latitude={lat}"));
                            if (lon > 180 || lon < -180) result.Errors.Add(Resources.ErrorCodes.Messaging.InvalidGeoLocationFormation.ToErrorMessage($"Key={Key},Longitude={lon}"));
                        }
                        else
                        {
                            result.Errors.Add(Resources.ErrorCodes.Messaging.InvalidGeoLocationFormation.ToErrorMessage($"Key={Key},Value={Value}"));
                ...
                case ParameterTypes.Integer:
                    if (!int.TryParse(Value, out int intValue)) result.Errors.Add(Resources.ErrorCodes.Messaging.ValueOnUnitSetnotDouble.ToErrorMessage($"Key={Key},Value={Value}"));
                    break;
                case ParameterTypes.State:
                    if (EntityHeader.IsNullOrEmpty(UnitSet)) result.Errors.Add(Resources.ErrorCodes.Messaging.StateSetSpecifiedButNotProvided.ToErrorMessage($"Key={Key}"));
                    if (!StateSet.Value.States.Where(state => state.Key == Value).Any())
                    {
